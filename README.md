...
- `batch_size=32`：如果显存足够，可以继续提高到 `64`。
- `d_model=128`
- `d_ff=256`
- `modes=128`
- `e_layers=2`
...
- `batch_size`：一次送进模型的样本窗口数量。越大越吃显存，训练通常更稳定、更快；不改变模型能力。服务器显存够可以试 `32`、`64`，不够就降到 `16` 或 `8`。
...
```text
Fourier + d_model=128 + d_ff=256 + modes=128 + e_layers=2 + batch_size=32 + full_inference
```

如果显存充足，再只把 `batch_size` 从 `32` 提到 `64`。如果想继续提升模型容量，第二轮再考虑 `d_model=256`、`d_ff=512`，不要和版本切换同时改。
